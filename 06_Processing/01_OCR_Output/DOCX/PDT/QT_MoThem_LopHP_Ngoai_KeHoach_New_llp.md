---
document_key: ctu-pdt-qt-mothem-lophp-ngoai-kehoach-new
version_key: ctu-pdt-qt-mothem-lophp-ngoai-kehoach-new-5e981c5accb4
title: "ĐƠN ĐỀ NGHỊ MỞ LỚP HỌC PHẦN"
document_type: don
domain: hoc_vu
issuing_authority: "Đại học Cần Thơ"
responsible_department:
signer_name: ""
audience:
  - student
code: ""
issued_date:
effective_date:
is_latest: true
ocr_status: done
review_status: approved
rag_status: not_indexed
status_note: ""
source_url: ""
source_path: "../Dataset/02_Attachments/DOCX/PDT/QT_MoThem_LopHP_Ngoai_KeHoach_New.doc"
canonical_markdown_path: "../Dataset/06_Processing/01_OCR_Output/DOCX/PDT/QT_MoThem_LopHP_Ngoai_KeHoach_New_llp.md"
file_type: doc
accessed_date: 2026-05-31
language: vi
checksum: 5e981c5accb4ab2f0295925567eb55b9304b5379f3f9ed771933cf46deb7af20
parser: llamaparse_postprocessed
ocr_engine: "LlamaParse API"
created_at: "2026-07-09T10:15:59+07:00"
updated_at: "2026-07-09T10:15:59+07:00"
notes: ""
---
1

<!-- page: 1 -->

**CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM**
**Độc lập – Tự do – Hạnh phúc**

# ĐƠN ĐỀ NGHỊ MỞ LỚP HỌC PHẦN

## HỌC KỲ ……, NĂM HỌC 20 ….. - 20…..

* **Kính gửi**:
  * - Lãnh đạo Khoa/Viện/ Bộ môn     ;
  * - Trưởng/Phó bộ môn:

Tôi tên:      									MSSV:
Là sinh viên ngành:      							Khóa:
Hệ đào tạo:      								Số điện thoại:
Thuộc Khoa (Trường/Viện/Bộ môn):

Tôi đại diện cho nhóm sinh viên có nhu cầu, kính đề nghị Lãnh đạo Khoa (Trường/Viện), Bộ môn (Khoa) quản lý học phần, xem xét mở lớp học phần:

Tên học phần:      								; Mã số HP:     ;
Số tín chỉ:      	;
Học kỳ:     	, năm học 20      -       20
Thời khóa biểu đề nghị:
 	- Thứ:      ; tiết:      hoặc - Thứ:      ; tiết:
    	- Thứ:      ; tiết:      hoặc - Thứ:      ; tiết:
Lý do:

Chúng tôi cam kết đã lập kế hoạch học tập và sẽ đăng ký học phần đúng theo quy định.

Trân trọng kính chào./.

Cần Thơ, ngày      tháng      năm 20.....
**Người làm đơn**
(Ký và ghi rõ họ tên)

**Khoa (Trường/Viện) quản lý học phần**
**Trưởng Khoa (Trường/Viện)**
(Ký và ghi rõ họ tên)

**Bộ môn (Khoa) quản lý học phần**
(Ghi họ tên và mã số GV được phân công)
**Trưởng bộ môn (Khoa)**
(Ký và ghi rõ họ tên)
